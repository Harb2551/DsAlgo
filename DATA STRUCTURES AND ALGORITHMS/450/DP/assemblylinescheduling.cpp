...
ll min_time(vvl a,vvl t,vl e,vl st,ll n){
  ll dp1[n+1];
  ll dp2[n+1];
  dp1[0]=st[0]+a[0][0];
  dp2[0]=st[1]+a[1][0];
  for(auto i=1;i<n;i++){
    dp1[i] = a[0][i] + min(dp1[i-1],dp2[i-1]+t[1][i]);
    dp2[i] = a[1][i] + min(dp2[i-1],dp1[i-1]+t[0][i]);
  }
  return min(dp1[n-1]+e[0],dp2[n-1]+e[1]);
}
