...
Node* construct(int pre[],int &i,int size,int ma,int mi){
    if(i<size and pre[i]>mi and pre[i]<ma){
        Node *temp = newNode(pre[i++]);
        temp->left = construct(pre,i,size,pre[i-1],mi);
        temp->right = construct(pre,i,size,ma,pre[i-1]);
        return temp;
    }
    return NULL;
}
...
Node* post_order(int pre[], int size)
{
    //code here
    int i = 0;
    Node *root = construct(pre,i,size,INT_MAX,INT_MIN);
    return root;
}
